...
export function getSessionFromReq(req: ExpressRequest): Session {
...
export class Controller {
  ...
  constructor() {
    ...
    this.roomService = new RoomServiceClient(httpUrl, process.env.LIVEKIT_API_KEY!, process.env.LIVEKIT_API_SECRET!)
  }
  ...
  async createStream({ metadata, room_name: roomName }: CreateStreamParams): Promise<CreateStreamResponse> {
    const at = new AccessToken(process.env.LIVEKIT_API_KEY!, process.env.LIVEKIT_API_SECRET!, {
      identity: metadata.creator_identity,
    })
